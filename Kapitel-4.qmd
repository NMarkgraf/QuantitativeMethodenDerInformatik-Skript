...
@gig_VarianzBias zeigt exemplarisch, was bei einer Messung *schief* gehen kann. Entweder streut das Messergebnis (Varianz) oder die Messung hat eine Abweichung (Verzerrung oder Bias), beispielsweise eine Waage, die verstellt ist.
...
Die metrische Skala wird auch als quantitative Skala bezeichnet. Wenn Sie nur die die Abstände inhaltlich sinnvoll interpretiert können, liegen die Daten auf einer **Intervallskala** vor, ein Beispiel wäre die Temperatur in °C. Können auch die Quotienten sinnvoll interpretiert werden, liegen die Daten auf einer **Verhältnisskala** vor, z. B. Größe in cm, Umsatz in Euro. Beispiele: 2 m ist doppelt so lang wie 1 m, aber 20 °C ist nicht doppelt so warm wie 10 °C -- inhaltlich ist die zweite Aussage nicht sinnvoll. Im Fall der Intervallskala ist der Nullpunkt gesetzt, während es bei der Verhältnisskala einen natürlichen Nullpunkt gibt. Daten auf der Intervallskala können Sie mit Faktor und Offset umrechnen (z. B. von der Celsius- zur Fahrenheitskala), während Sie Daten auf der Verhältnisskala nur mit einem Faktor umrechnen können (z. B. Meter in Inch).

::: {.callout-tip .Tipp}
## Tipp

Eigenschaften werden in *Kategorien* angegeben, daher *kategoriale* Daten. Wenn Sie etwas *messen* können (quasi mit dem Maßband oder dem Zollstock), sind es *metrische* Daten.
:::
...
Metrische Daten werden zusätzlich in **stetig** und **diskret** unterteilt. Stetige Daten können beliebig viele Zwischenwerte (Nachkommastellen) haben, je nach Genauigkeit des Messintruments, z. B. Länge. Diskrete Daten haben abzählbar viele Zwischenwerte, meist sind sie ganzzahlig. Ein Beispiel dafür ist die Anzahl Studierender in einer Veranstaltung. Manchmal gibt es auch Daten, die z. B. auf zwei Nachkommastellen diskret sind wie Geldbeträge in Euro. Da es dann aber sehr viele Stufen gibt, werden sie als **quasi-stetig** bezeichnet und wie stetige Werte behandelt.
...
Sie finden an vielen Stellen Verweise auf Songs und Videos. Wenn Sie Spaß daran haben, hören oder schauen Sie sich diese an. Es lockert den Ablauf auf und bewirkt eine zusätzliche Verfestigung des Lernstoffes.
:::
...
: Exemplarischer Datensatz{#tab_Data}
:::

@tab_Data zeigt einen exemplarischen Datensatz. Ein Datensatz ist meist wie ein Tabellenkalkulationsblatt aufgebaut. Daten werden häufig auch in Formaten abgespeichert, die von Tabellenkalkulationsprogrammen (wie Excel, Numbers o. ä.) gelesen werden können.

Die Spalten entsprechen in der Regel einer Variablen, hier Name, Geschlecht und Alter, eine Zeile zeigt eine Beobachtung. Häufig ist die erste Zeile in einem Datensatz die Kopfzeile, gibt also die Namen der Variablen aus.

::: {.callout-note .Aufgabe}
Lesen Sie Kapitel 1 aus dem Buch *Introduction to Modern Statistics* \[6\]. Sie können es hier herunterladen: <https://openintro-ims.netlify.app/index.html>
:::
...
-   **Kovariable:** eine oder mehrere weitere Variablen, die ebenfalls die abhängige Variable oder den Zusammenhang zwischen der abhängigen und den unabhängigen Variablen beeinflussen. Diese werden auch als Störvariablen bezeichnet ( $𝑧$ ).
...
Die Modellformulierung in der Statistik lautet $𝑦 ∼ 𝑥$ . Im Unterschied zur mathematischen Funktion \$𝑦 = 𝑓 (𝑥) \$ kann es in der statistischen Formulierung für ein $𝑥$ auch mehrere $𝑦$ -Werte geben.

Dass Sie bei der Auswahl der Kovariablen aufpassen müssen, zeigt @fig_UBC1, in der es um Zulassungen an der University of California Berkeley im Zusammenhang mit dem Geschlecht geht (Bickel, Hammel und O'Connell @bickel1975).


```{r}
#| label: fig_UCB1
#| caption: "Zulassungen an der UC Berkeley in Abhängigkeit vom Geschlecht: ohne Kovariable"
#| echo: false
#| fig.width: 5
#| fig.asp: 1
#| out.width: 7cm
...
Erst die Aufteilung nach der Kovariable Department (@fig_UCB2) zeigt, dass Frauen und Männer ausgeglichen zugelassen wurden, im Department A sogar relativ mehr Frauen. Die Verzerrung im Gesamtbild ergibt sich durch die Wahl der Departments: Frauen haben sich eher dort beworben, wo die Ablehnungsquote höher ist.

```{r chapQDGl-figUCB2, fig = FALSE, echo = FALSE, fig.width = 6.5, fig.asp = 0.8, out.width = "10cm", out.extra = "trim = 5mm 10mm 5mm 0mm, clip"}
#| caption: "Zulassungen an der UC Berkeley in Abhängigkeit vom Geschlecht: mit Kovariable Department"
#| label: fig_UCB2}
...
@fig_Sample zeigt die verschiedenen Erhebungsmöglichkeiten schematisch.

```{r chapQDGl-figSample, fig = TRUE, echo = FALSE, fig.width = 5, fig.asp = 0.6, out.width = "10cm"}
#| caption: "Verschiedene Erhebungsmethoden für Stichproben"
#| label: fig_Sample}
...
::: Hinweis
Die Abbildung zeigt einen sogenannten Directed Acyclic Graph, kurz DAG. Damit können kausale Zusammenhänge dargestellt werden. Mehr dazu im Kapitel 9 (Einführung in die kausale Modellierung).
:::
...
Der Graf zeigt den **datengenerierenden Prozess** in einer Beobachtungsstudie und einem randomisierten Experiment. Im letzteren weist die Dozentin den Studierenden **zufällig** eine Lernzeit zu.
