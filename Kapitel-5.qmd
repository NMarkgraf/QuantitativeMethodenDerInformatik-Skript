...
Um Anteile (englisch: *proportion*) darzustellen, können Sie die Option `format = "proportion"` ergänzen, für Prozentwerte entsprechend `format = "percent"`.
...
Bei den meinsten Optionswerten, die als Zeichenkette in Anführungzeichen eingegen werden, reicht es, weniger Buchstaben anzugeben, hier z. B. "prop" oder "per". Es muss nur eindeutig sein.
:::
...
::: {.callout-tip .Tipp}
## Tipp
`mosaic::` muss nicht unbedingt davor angegeben werden, aber so wird sichergestellt, dass nach der Hilfe für die Funktion aus dem Paket `mosaic` gesucht wird.
...
Auch hier gibt es ein Tastenkürzel:
Windows AltGr - <, Mac Alt - 7. 
Das sind jetzt aber auch schon alle (zumindest für die Formelnsyntax).
:::
